...
## 41. Retry mechanism for invalid `apply_patch` format: Anthropic API constraints (A1–A6)

### Description
When Anthropic generates an `apply_patch` tool_use with invalid format (not starting with `*** Begin Patch` and not parseable as unified diff), the proxy should retry by: (1) detecting invalid format, (2) suppressing the failed output from Codex, (3) constructing a retry request with the failed assistant message + a tool_result error, and (4) streaming the retry response to Codex as a continuation. This entry covers research questions A1–A6 about Anthropic Messages API constraints.

### Result

**A1: Tool-use conversation pattern for retry.** The Anthropic tool-use conversation pattern is: assistant message with `tool_use` blocks → user message with `tool_result` blocks. The `tool_result` block format is `{"type": "tool_result", "tool_use_id": "<id>", "content": "<string or content blocks>", "is_error": true/false}`. The `tool_use_id` must match the `id` from the assistant's `tool_use` block. Content can be a plain string or a list of nested content blocks (`text`, `image`, `document` types). For retry, the proxy would set `is_error: true` with an error message explaining the format problem [1][2].

**A2: tool_result content format for error.** The `content` field in `tool_result` can be a string (e.g., `"content": "Error: invalid patch format"`) or a list of content blocks (e.g., `"content": [{"type": "text", "text": "Error: invalid patch format"}]`). Both forms are valid. The `is_error: true` field signals to Anthropic that the tool execution failed. Anthropic will then attempt to correct its output in the next response [1][2].

**A3: Multiple tool_use blocks in a single response.** Anthropic supports parallel tool use by default — a single assistant response can contain multiple `tool_use` content blocks. For retry, if the assistant response contains multiple `tool_use` blocks (e.g., `exec_command` + `apply_patch`), the retry request must include `tool_result` blocks for ALL of them in a single user message. You cannot include text before `tool_result` blocks in the content array. For `exec_command` (non-failed), the proxy would need to fabricate a `tool_result` or only retry if `apply_patch` is the sole tool_use [1][3].

**A4: Thinking blocks in the retry assistant message.** Anthropic documentation states: "During tool use, you must pass thinking blocks back to the API for the last assistant message. Include the complete unmodified block back to the API to maintain reasoning continuity." The entire sequence of consecutive thinking blocks must match the outputs generated by the model during the original request; they cannot be rearranged or modified. If the original response had thinking blocks before the tool_use, the retry assistant message MUST include those thinking blocks (with their signatures) unmodified [4].

**A5: redacted_thinking handling in retry.** If the original response contained `redacted_thinking` blocks, they must also be passed back in the retry assistant message. Anthropic documentation warns: "If your code filters content blocks by type (for example, `block.type == 'thinking'`) when round-tripping responses with tool use, also include `redacted_thinking` blocks. Filtering on `block.type == 'thinking'` alone silently drops `redacted_thinking` blocks and breaks the multi-turn protocol." The `redacted_thinking.data` field is opaque encrypted content that must be preserved [4].

**A6: tool_use_id matching requirement.** The `tool_use_id` in the retry's `tool_result` must exactly match the `id` from the assistant's `tool_use` block. This is a hard requirement — mismatched IDs cause a 400 error. Since the proxy generates its own `fc_id`/`call_id` IDs for the Responses API, the original Anthropic `toolu_xxx` ID must be preserved for constructing the retry request [1][2].

**Key constraint for retry feasibility:** The tool_choice parameter is restricted when thinking is enabled — only `auto` or `none` are allowed. The retry request cannot force `tool_choice: {type: "tool", name: "apply_patch"}` to ensure Anthropic retries with apply_patch. Anthropic may respond with text instead of a tool call in the retry [4].

### Reference
- [1] Anthropic Handle Tool Calls docs — https://platform.claude.com/docs/en/agents-and-tools/tool-use/handle-tool-calls
- [2] Anthropic Messages API — https://platform.claude.com/docs/en/api
- [3] Anthropic Parallel Tool Use docs — https://platform.claude.com/docs/en/agents-and-tools/tool-use/parallel-tool-use
- [4] Anthropic Extended Thinking docs — https://platform.claude.com/docs/en/build-with-claude/extended-thinking

---

## 42. Retry mechanism: streaming continuation feasibility (B7–B8)

### Description
How should the retry response appear to Codex CLI? Should it be a seamless continuation of the original SSE event stream, or a new response with a new response ID? This entry covers questions B7–B8 about SSE event sequence continuity and response ID handling.

### Result

**B7: SSE event sequence continuity — the retry MUST be a new response.** The Responses API streaming lifecycle has strict ordering requirements: `response.created` → `response.in_progress` → output items → `response.completed`/`response.incomplete` → `[DONE]` [1]. A retry cannot be spliced into the middle of an existing response's event stream because:

1. The original response has already emitted `response.created` and `response.in_progress` events (and possibly output items for text/thinking that preceded the apply_patch).
2. The original response's output items are now invalid because the apply_patch failed.
3. Codex CLI maintains state based on the event sequence — a new `response.created` mid-stream would be confusing.

**However, the current architecture provides a critical simplification:** For `apply_patch` (custom_tool_call), the proxy already does NOT forward `FunctionCallArgumentsDelta` events to Codex — input_json_delta events are accumulated silently (see `current_is_custom` flag in `src/conversion/response.rs` line 421). Codex only receives the final `OutputItemDone` event at `content_block_stop` time with the converted patch. This means:

- If the invalid format is detected at `content_block_stop` (when `convert_to_freeform_patch` is called), Codex has not yet received any output item for the failed apply_patch.
- If text or thinking content preceded the apply_patch, those output items HAVE already been forwarded to Codex — they cannot be recalled.

**Two retry strategies emerge:**

*Strategy A (Simple — retry only if apply_patch is the first output):* If the response starts with apply_patch (no text/thinking before it), suppress the output entirely, close the original response with `response.completed` containing only the pre-patch output items, then start a new response for the retry. But since Codex has not seen any output items for apply_patch, the proxy can simply suppress the failed tool_use output, send the original response's `response.completed` without the failed item, then start a fresh response.

*Strategy B (Complex — retry even with preceding content):* If text/thinking preceded the apply_patch, those items have already been streamed to Codex. The retry would need to produce a new response that either (a) re-includes the same text/thinking output (duplicating what Codex already saw), or (b) only contains the corrected tool call. Option (b) is problematic because Codex would see two partial responses for what it considers a single request.

**Design recommendation:** Strategy A is strongly preferred. Retry is only attempted when `apply_patch` is detected as the FIRST (or only) output item, or more precisely, when the proxy has not yet forwarded any content to Codex. If text/thinking has already been forwarded, the retry should NOT be attempted — fall through to the normal conversion (which may produce an invalid patch that Codex will reject, but this is the safer failure mode).

**B8: Response ID continuity.** The retry response MUST use a new, different response ID. Reasons:

1. Anthropic generates a new `msg_xxx` ID for each Messages API call. The retry is a new API call and gets a new ID.
2. The Responses API requires `response.created` to carry a unique response ID [1].
3. Codex CLI may use the response ID for caching, logging, or correlation.
4. Using the same response ID for two different response payloads would be semantically incorrect.

The proxy generates a `rs_xxx` ID for reasoning items and `fc_xxx`/`call_xxx` IDs for function calls. These must be generated fresh for the retry response. The signature cache keys from the retry will use the new reasoning IDs.
...
## 43. Retry mechanism: edge cases (C9–C13)

### Description
Edge cases for the retry mechanism: responses with text/thinking before apply_patch, responses without apply_patch in retry, content after the tool call, token/billing implications, and multiple apply_patch calls in a single response.

### Result

**C9: Retry when text or thinking precedes apply_patch.** If Anthropic's response contains text or thinking blocks before the apply_patch tool_use block, those blocks have already been forwarded to Codex as `OutputItemAdded`/`OutputTextDone`/`ReasoningSummaryTextDelta` events by the time apply_patch's `content_block_stop` is reached. These events cannot be "unsent." Three options:

1. **Don't retry (safest):** If any output items have been forwarded to Codex before the apply_patch failure, skip retry. Accept the invalid patch and let Codex handle it. This is the simplest and least risky approach.
2. **Send a new response with duplicated content:** Emit `response.completed` for the original (with all items including the failed tool_use), then emit a second `response.created` → `response.in_progress` → ... → `response.completed` for the retry. This means Codex receives TWO responses for one request. Codex CLI is designed for one response per request and would likely treat the second as an error or ignore it.
3. **Buffer all output until apply_patch completes:** Don't forward ANY events until the apply_patch is resolved. This adds latency for all responses (even successful ones) and requires buffering the entire response in memory.

**Design recommendation:** Option 1 — do NOT retry if any output items have been forwarded. Only retry when the apply_patch is the first output and no events have been sent to Codex yet. In the current architecture, `apply_patch` as a custom_tool_call accumulates silently, so detection happens at `content_block_stop`. If `output_items` is empty at that point (no text/thinking items were emitted), retry is safe.

**C10: Anthropic responds with text instead of apply_patch in retry.** After receiving a `tool_result` with `is_error: true`, Anthropic may choose to respond with text explaining the error rather than making another tool call. This is valid behavior. Since tool_choice is restricted to `auto` or `none` when thinking is enabled (see A6 in entry #41), the proxy cannot force Anthropic to produce another apply_patch. The retry response must be handled as-is — if it contains text, forward it to Codex as a regular text output item. If it contains a corrected apply_patch, forward that. The proxy should not loop-retry indefinitely.

**C11: Content after the tool call.** In Anthropic's streaming response, a `tool_use` content block followed by additional content (text or more tool_use blocks) is possible. If the response has: `[thinking, tool_use(apply_patch), text]` and the apply_patch fails, the text block comes AFTER the tool_use in the stream. By the time `content_block_stop` fires for the apply_patch (when invalid format is detected), the text block has not yet been received from upstream. This means:
- The proxy could stop consuming the upstream stream at this point (disconnect).
- The retry request would need to include the already-received thinking blocks but NOT the not-yet-received text.
- The original response would be incomplete (missing the trailing text), but since it's being suppressed, this is fine.

However, this scenario is unlikely in practice — Anthropic typically does not produce text after a tool_use block when `stop_reason` is `tool_use`.

**C12: Token/billing implications.** Every retry attempt is a new Anthropic API call that incurs full input and output token costs. The retry request includes:
- The entire original conversation history (all input items).
- The failed assistant message (with thinking + tool_use blocks).
- A new user message with the tool_result error.

This roughly doubles the token cost for a retried request. If thinking is enabled, the input includes the failed thinking blocks (potentially thousands of tokens). Multiple retries compound this cost linearly. The proxy should limit retries (recommend: maximum 1 retry per request). The billed output tokens include both the failed response AND the retry response.

**C13: Multiple apply_patch calls in a single response.** Anthropic supports parallel tool use and could theoretically return multiple `apply_patch` tool_use blocks in a single response. In practice, Anthropic rarely calls the same tool multiple times in parallel, but it is possible. If this occurs:
- Each apply_patch is handled as a separate ToolUse block with its own content block index.
- The proxy would need to validate each one independently.
- If one fails and one succeeds, retrying is complex — the retry would need tool_results for both (one success, one error).
- **Design recommendation:** If multiple apply_patch calls are present, do NOT retry — too many edge cases. Accept the mixed result and let Codex handle it.

### Reference
- [1] Anthropic Handle Tool Calls docs — https://platform.claude.com/docs/en/agents-and-tools/tool-use/handle-tool-calls
- [2] Anthropic Parallel Tool Use docs — https://platform.claude.com/docs/en/agents-and-tools/tool-use/parallel-tool-use
- [3] Anthropic Extended Thinking docs — https://platform.claude.com/docs/en/build-with-claude/extended-thinking
- [4] OpenAI Responses streaming events reference — https://developers.openai.com/api/reference/resources/responses/streaming-events/

---

## 44. Retry mechanism: implementation concerns in current architecture (D14–D16)

### Description
How would the retry mechanism integrate with the current proxy architecture? This entry covers the connection lifecycle (D14), StreamingState reuse (D15), and cache_control implications (D16).

### Result

**D14: Connection lifecycle integration.** The current streaming pipeline in `src/router.rs` works as follows:
1. `mpsc::channel(64)` creates a sender `tx` and receiver `rx`.
2. `tx` is moved into a spawned Tokio task that consumes the upstream SSE stream.
3. `rx` is wrapped in `ReceiverStream` and returned as the HTTP response body.
4. The streaming task loops: receives Anthropic events → converts via `StreamingState::process_event` → sends Responses API events via `tx`.
5. After the loop ends, signatures are drained from StreamingState into the cache.
6. `event_source.close()` disconnects the upstream.

For retry, the implementation would need to:

1. **Detect invalid format:** At `content_block_stop` for an apply_patch ToolUse, `convert_to_freeform_patch` is called. Currently it converts unified diff to freeform format. If the input is neither `*** Begin Patch` nor valid unified diff, the result would be a malformed patch. Detection would need a new validation function that returns `Result` instead of always succeeding.

2. **Check retry eligibility:** Only retry if: (a) no output items have been forwarded to Codex yet (`output_items` is empty or contains only items not yet sent via `tx`), (b) this is a custom_tool_call (apply_patch), (c) retry count has not been exceeded.

3. **Suppress the failed output:** Do not emit `OutputItemDone` for the failed apply_patch. This is straightforward — simply return no events from `handle_content_block_stop` instead of the normal events.

4. **Close the original response:** Send `response.completed` with empty output, then `[DONE]`. Drop `tx` to close the downstream connection... BUT this means Codex receives an empty response, which it would treat as a failure. Alternatively, don't close the downstream connection — keep `tx` alive for the retry response.

**The key architectural challenge:** The current design sends events via `tx` as they arrive. For retry, the proxy needs to:
- NOT send the failed output items via `tx`.
- Continue the upstream stream consumption to drain the remaining events (or disconnect upstream).
- Construct a new Anthropic Messages API request.
- Create a new `EventSource` for the retry request.
- Feed the retry's SSE events through a new `StreamingState`.
- Forward the retry's converted events via the same `tx`.

This means the streaming task needs to be extended with a retry loop:
```
loop {
    // original stream
    while let Some(event) = event_source.next().await {
        process event...
        if invalid_format_detected && retry_eligible {
            drain remaining upstream events
            break
        }
    }
    if retry_needed {
        build retry request
        event_source = new EventSource(retry_request)
        streaming_state = new StreamingState(...)
        retry_needed = false
        continue  // loop back to consume new stream
    } else {
        break
    }
}
...
This is architecturally feasible within the existing spawned task — no changes to the `mpsc` channel or the downstream connection are needed. The same `tx` continues to be used.

**D15: StreamingState reuse.** The `StreamingState` struct accumulates per-response state (output items, token counts, stop reason, tool_use_map, etc.). For a retry, a new `StreamingState` must be created because:
- The retry response has a different Anthropic `msg_xxx` ID.
- Token counts start fresh.
- The `fc_counter` resets to 0 (new set of `fc_xxx`/`call_xxx` IDs).
- The `output_items` array is empty.
- Signatures from the failed response may need to be cached before creating the new state.

However, the `namespace_registry` and echo fields (`model`, `tool_choice_echo`, `instructions_echo`, `parallel_tool_calls_echo`) should be preserved from the original state. These are per-request, not per-response.

The `signature_store` from the failed response should be drained and written to cache before creating the new StreamingState, even though the retry will likely generate its own thinking blocks. The failed response's thinking blocks were generated by the model and their signatures are valid — they just won't be used because the conversation history is being reconstructed with the retry.

**D16: Cache control implications.** Cache invalidation patterns with retry:

1. **Prompt caching:** The retry request includes the same system prompt, tools, and conversation history as the original request, PLUS the failed assistant message and a tool_result. This means the cache prefix (system + tools + early messages) should still hit from the original request's cache entry — Anthropic caches prefixes [3].

2. **Thinking parameter changes:** The retry must use the same `thinking` configuration as the original request. Changing thinking parameters between the original and retry would invalidate the message cache breakpoints. The proxy already preserves the thinking configuration from the original request, so this is naturally satisfied [3].

3. **cache_control markers:** If the original request had `cache_control: {type: "ephemeral"}` markers on content blocks, the retry request must include them in the same positions. The proxy's `convert_input_items` function handles this, and the retry would use the same conversion path.

4. **Practical cache benefit:** The retry request reuses the same system prompt and tools, which is typically the largest part of the input. The cache hit on these prefixes can save significant input token costs on the retry. The new content (failed assistant message + tool_result) is appended after the cached prefix.

5. **New response, new signatures:** The retry response generates new thinking signatures. These must be cached under the retry's reasoning IDs, not the failed response's IDs. The signature cache insertion happens after the retry stream ends.

### Reference
- [1] Source code: `src/router.rs` (streaming task, lines 264–418)
- [2] Source code: `src/conversion/response.rs` (StreamingState, handle_content_block_stop)
- [3] Anthropic Extended Thinking with Prompt Caching — https://platform.claude.com/docs/en/build-with-claude/extended-thinking (see "Cache invalidation patterns" section)

---

## 45. Retry mechanism: overall feasibility assessment and design recommendation

### Description
Synthesis of findings from entries #41–#44. Is the proposed retry mechanism feasible? What are the constraints, risks, and recommended design?

### Result

**Feasibility: FEASIBLE with constraints.** The retry mechanism is architecturally sound and implementable within the current codebase, but has important constraints that limit when it can be applied.

**When retry CAN be applied (safe conditions):**
1. The apply_patch tool_use is the FIRST output item (no text/thinking blocks before it). This is verified by checking `output_items.is_empty()` or that no events have been forwarded via `tx` before the tool_use's `content_block_stop`.
2. Only ONE apply_patch is present in the response (no parallel tool use complication).
3. The retry count for this request is 0 (first attempt only).
4. The format is unambiguously invalid (not starting with `*** Begin Patch` AND not parseable as unified diff with actual file changes).

**When retry MUST NOT be applied:**
1. Text or thinking output items have already been forwarded to Codex.
2. Multiple tool_use blocks are present in the response (parallel tool use).
3. The response contains an `exec_command` alongside the `apply_patch` (cannot fabricate a tool_result for exec_command).
4. Maximum retries exceeded (recommend limit: 1).

**Critical architectural simplification in current code:** The `current_is_custom` flag in `StreamingState` causes `apply_patch` input to be accumulated silently — no `FunctionCallArgumentsDelta` events are forwarded to Codex. The first and only event Codex sees for apply_patch is `OutputItemDone` at `content_block_stop`. This means invalid format detection at `content_block_stop` is the perfect interception point — Codex has not yet received any data about the failed tool call.

**Construction of the retry request:**
1. Clone the original request's `messages` array.
2. Append an assistant message with the failed thinking blocks (with signatures) + tool_use block (with original `toolu_id`).
3. Append a user message with `tool_result` (`tool_use_id` = original `toolu_id`, `is_error: true`, `content` = error message describing the format issue).
4. Preserve the same `system`, `tools`, `thinking`, `output_config`, and all other request parameters.
5. Send the retry request to Anthropic and stream the response through a new `StreamingState`.

**Retry response handling:**
- The retry response uses a new Anthropic `msg_xxx` ID and new proxy-generated response ID.
- The retry response's events are forwarded via the same `tx` channel — Codex sees one continuous SSE stream.
- The retry response may contain text instead of apply_patch (Anthropic is not forced to retry the tool call). This must be handled gracefully.
- If the retry response also produces an invalid apply_patch, do NOT retry again — forward the invalid patch to Codex (Codex will reject it, which is the correct failure mode).

**Token cost:** Each retry roughly doubles the request cost because the full conversation history is sent again plus the failed assistant message. For coding tasks where apply_patch is common and format errors are rare (Anthropic models are generally good at both formats), the cost impact is minimal.

**Risks:**
1. Anthropic may respond with text instead of a corrected apply_patch (mitigated by `tool_choice: auto` — Anthropic is likely to retry the tool call given the error context).
2. Cache invalidation if thinking parameters change between retries (mitigated by preserving the exact same thinking configuration).
3. Increased latency (the retry adds a full round-trip to Anthropic before Codex sees any output).
4. The retry response may produce a different tool call entirely (mitigated by accepting whatever Anthropic returns).

**Recommended implementation approach:**
1. Add a `validate_patch_format` function that returns `Result<String, String>` instead of the current `convert_to_freeform_patch` which always succeeds.
2. In the streaming task's loop, detect retry-eligible failures at `content_block_stop` for ToolUse.
3. Implement a single-retry loop in the streaming task (the `mpsc::Sender` stays alive across retries).
4. Log retry attempts for observability.
5. Make retry opt-in via configuration flag (default off).
